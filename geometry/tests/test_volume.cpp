...
TEST_CASE("volume", "[simplex]") {
    {
        ...
        auto identity_test = []<int N>(std::integral_constant<int, N>) {
            ...
            V.col(0) = zipper::expression::nullary::Constant<float>(0);
            W.col(0) = zipper::expression::nullary::Constant<float>(0);
            V.template slice<zipper::static_index_t<1>, zipper::full_extent_t>() = zipper::expression::nullary::Constant<float>(0);
            W.template slice<zipper::static_index_t<1>, zipper::full_extent_t>() = zipper::expression::nullary::Constant<float>(0);
            for (int j = 0; j < N; ++j) {
                V(j, j + 1) = 1;
                W(j, j + 1) = 1;
            }
