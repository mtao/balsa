...
template<int C, typename MatType>
    requires concepts::ColCompatible<C, MatType>
constexpr bool col_check(const MatType &N) {
    return col_check_oneof(N, std::integer_sequence<int, C>{});
}

template<int R, typename MatType>
    requires concepts::RowCompatible<R, MatType>
constexpr bool row_check(const MatType &N) {
...
template<int C, typename MatType>
    requires concepts::ColCompatible<C, MatType>
constexpr void col_check_with_throw(const MatType &N) {
    if (!col_check<C>(N)) {
        throw std::invalid_argument(std::format("Col check: wrong size, got {} expected {}", N.cols(), C));
    }
}

template<int R, typename MatType>
    requires concepts::RowCompatible<R, MatType>
constexpr void row_check_with_throw(const MatType &N) {

    if (!row_check<R>(N)) {
        throw std::invalid_argument(std::format("Row check: wrong size, got {} expected {}", N.rows(), R));
    }
}
template<int R, int C, typename MatType>
...
void col_check_with_throw(const MatType &N, std::integer_sequence<int, C...> Seq) {
    if (!col_check_oneof(N, Seq)) {
        std::string expected;
        ((expected += (expected.empty() ? "" : ", ") + std::to_string(C)), ...);
        throw std::invalid_argument(std::format("Col check: wrong size, got {} expected one of ({})", N.cols(), expected));
    }
}
...
void row_check_with_throw(const MatType &N, std::integer_sequence<int, R...> Seq) {
    ...
    if (!row_check_oneof(N, Seq)) {
        std::string expected;
        ((expected += (expected.empty() ? "" : ", ") + std::to_string(R)), ...);
        throw std::invalid_argument(std::format("Row check: wrong size, got {} expected one of ({})", N.rows(), expected));
    }
}
template<int... R, int... C, typename MatType>
